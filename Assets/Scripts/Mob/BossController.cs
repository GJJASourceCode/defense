...
public class BossController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector2.Distance(moveTarget, transform.position) <= 0.1f)
        {
            ...
            mob.tilePosition = graph[currentPos.x - pathFinding.ground.cellBounds.xMin][currentPos.y - pathFinding.ground.cellBounds.yMin];
            // FindObjectOfType<SpawnManager>().SpawnObject(startPos, testPrefab);
            ...
            {   
                ...
                {
                    temp = pathFinding.ground.CellToWorld(new Vector3Int(targetPos.x, currentPos.y, 0));
                   
                }
                else{
                    temp = pathFinding.ground.CellToWorld(new Vector3Int(currentPos.x, targetPos.y, 0));
                }
                moveTarget = new Vector2(temp.x, temp.y + 0.15f);
            }
        }
