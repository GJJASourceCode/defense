...
public class SpawnManager : MonoBehaviour
{
    ...
    {
        if (
            tileIntPos.x < cellBounds.xMin
            || tileIntPos.x > cellBounds.xMax
            || tileIntPos.y < cellBounds.yMin
            || tileIntPos.y > cellBounds.yMax
            || !isBuilt[tileIntPos.x - cellBounds.xMin][tileIntPos.y - cellBounds.yMin]
        )
        {
            return null;
        }
    ...
    public GameObject SpawnObject(Vector3Int tileIntPos, GameObject prefab)
    {
        ...
        if (mobSpawner)
        {
            ...
            var startPos = pathFinding.ground.WorldToCell(
                mobSpawner.transform.position - Vector3.up * 1f
            );
